...
function cardsProductos(){
    for (const anillo of anillosApi){
        // Realizo desestructuracion del objeto y sus propiedades
        const {id , nombre, precio, foto} = anillo
    ...
    anillosApi.forEach((anillo)=>{
        document.getElementById(`btn${anillo.id}`).addEventListener("click", function(){ // le asignamos el evento de click y que con ese click agregue al carrito 
...
function agregarAlCarrito(anilloAgregado){
    ...
    contenidoCarro.innerHTML +=`
    <tr>
        <td>${anilloAgregado.id}</td>
        <td>${anilloAgregado.nombre}</td>
        <td>${anilloAgregado.precio}</td>
        <td><button class="btn btn-light" onclick="eliminar(event)">🗑️</button></td>
    </tr>    
    `;
